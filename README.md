...
The current `1.21.1` branch builds **FabricPlots 1.0.1 for Minecraft 1.21.1**. This patch focuses on
...
Requires **JDK 21**. Minecraft 1.21.1 is built with official Mojang mappings and Loom's remap plugin.
DraftSmith 1.0.1 is consumed from Maven Local, so publish its matching `1.21.1` branch first
(keep the two repositories as siblings):

```bash
(cd ../draftsmith && git switch 1.21.1 && ./gradlew publishToMavenLocal)
./gradlew build      # or: gradle build
```

Output for this branch: `build/libs/fabricplots-1.0.1+1.21.1.jar`.
