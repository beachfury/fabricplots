...
public final class PlotData {
    ...
    public boolean spawnHostile = false; // allow the biome's hostile (MONSTER) mobs to spawn on this plot (OFF until the owner opts in)
    public boolean spawnPassive = false; // allow the biome's passive mobs to spawn on this plot (OFF until the owner opts in)
    public int mobCap = -1;         // owner's tracked-mob cap per plot cell; -1 = use the server default (mob-cap-per-plot)
